...
@Component({
  selector: 'app-apply-again',
  templateUrl: './apply-again.component.html',
  styleUrls: ['./apply-again.component.scss'],
})
export class ApplyAgainComponent implements OnInit {
  //extraAmount = 0;
  notes: string = '';
  ...
  purchaseTypes: { label: string; value: number }[] = [];
    ...
    this.saveForm(isSave)
      .then(async (res) => {
        ...
        if (res?.message === 'Successfully saved') {
          ...
          // }
          this.router.navigate(['/dashboard/under-review']);
        }
      })
      .catch((err) => {
  ...
  getCandidateForms() {
    this.authSvc.getCandidateForms().subscribe((res) => {
      ...
      // Annual Target
      this.partnerSignUpForm?.controls?.unitsInNxtYr.setValue(
        String(res?.annual_target).replace(/>/g, '')
      );
